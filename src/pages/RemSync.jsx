...
export default function RemSync() {
  ...
  const steps = [
    { name: "Client Site", icon: Globe, color: "#6366f1", tech: "React / Vite" },
    { name: "Cloud Proxy", icon: Server, color: "#10b981", tech: "Vercel Serverless" },
    { name: "Bank Gateway", icon: Lock, color: "#f59e0b", tech: "OAuth 2.0 / REST" },
    { name: "R.E.M Logic", icon: Cpu, color: "#ec4899", tech: "Neural Parsing" }
  ];
  ...
  const handleSync = async () => {
    ...
    const logMessages = [
      "Initializing OAuth 2.0 Secure Handshake...",
      "Connecting to Vercel Serverless Edge...",
      "Requesting tokens from Garanti BBVA API Market...",
      "Parsing Sandbox AIS Data structure...",
      "R.E.M Engine: Normalizing raw bank payloads...",
      "Success: Secure data tunnel established."
    ];

    for(let i=0; i<4; i++) {
      setActiveStep(i + 1);
      addLog(logMessages[i]);
      await new Promise(r => setTimeout(r, 800));
    }
    addLog(logMessages[4]);
    await new Promise(r => setTimeout(r, 600));
    addLog(logMessages[5]);
    ...
    <motion.div initial="hidden" animate="show" style={{ flex: 1, display: 'flex', flexDirection: 'column', padding: '0 40px 60px 40px', overflowY: 'auto' }}>
      ...
      <div style={{ marginBottom: 40, marginTop: 40, textAlign: 'center' }}>
        <motion.div variants={fadeUp} style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderRadius: 100, background: 'rgba(16,185,129,0.1)', color: '#10b981', fontSize: 10, fontWeight: 900, letterSpacing: 2, marginBottom: 20 }}>
          <CheckCircle2 size={14} /> FULL STACK INFRASTRUCTURE ACTIVE
        </motion.div>
      ...
      <div style={{ maxWidth: 1000, margin: '0 auto', width: '100%' }}>
        ...
        {/* ENHANCED BRIDGE UI WITH PULSING */}
        <div className="glass" style={{ padding: '40px 48px', borderRadius: 32, border: '1px solid rgba(255,255,255,0.05)', marginBottom: 24, background: 'rgba(255,255,255,0.01)' }}>
                ...
                <div key={i} style={{ textAlign: 'center', flex: 1, position: 'relative' }}>
                   <motion.div 
                     animate={activeStep >= i + 1 ? {
                       scale: [1, 1.1, 1],
                       boxShadow: [`0 0 10px ${s.color}20`, `0 0 30px ${s.color}60`, `0 0 10px ${s.color}20`]
                     } : {}}
                   ...
                   <div style={{ fontSize: 10, fontWeight: 900, color: activeStep >= i + 1 ? '#fff' : 'var(--text2)', transition: 'all 0.5s ease' }}>{s.name.toUpperCase()}</div>
                   <div style={{ fontSize: 8, fontWeight: 800, color: 'var(--accent)', marginTop: 4, opacity: 0.6 }}>{s.tech}</div>
                   
                   {i < 3 && (
                     <div style={{ position: 'absolute', top: 30, right: '-50%', width: '100%', height: 1, background: 'rgba(255,255,255,0.05)', zIndex: 1 }}>
                        <motion.div 
                          initial={{ width: 0 }} 
           ...
           <div style={{ background: '#000', borderRadius: 16, padding: 20, fontFamily: 'monospace', minHeight: 120, border: '1px solid rgba(255,255,255,0.1)' }}>
              <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
                 ...
                 <span style={{ fontSize: 10, color: 'var(--text2)', marginLeft: 10, fontWeight: 800 }}>TECH_INSIGHT_V2.0</span>
              </div>
              <AnimatePresence mode="popLayout">
                {logs.length === 0 && <div style={{ color: 'rgba(255,255,255,0.2)', fontSize: 12 }}>Sistem boşta. Senkronizasyon bekleniyor...</div>}
                {logs.map((log, i) => (
                  <motion.div key={i} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} style={{ color: '#10b981', fontSize: 12, marginBottom: 4 }}>{log}</motion.div>
                ))}
              </AnimatePresence>
           </div>
        </div>
        ...
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
            <button onClick={handleSync} disabled={isSyncing} className="btn btn-primary" style={{ padding: '20px 56px', borderRadius: 16, fontSize: 16, fontWeight: 950, gap: 12 }}>
              ...
              {isSyncing ? 'Teknoloji Katmanları Bağlanıyor...' : 'Otonom Hattı Ateşle'}
            </button>
        </div>
