...
import { 
  ...
  Play, Smartphone, Fuel, Calendar, Upload, X, Search, Trash2
} from 'lucide-react';
...
export default function RemSync() {
  ...
  useEffect(() => {
    ...
    setSyncedData(existing);
  }, []);

  const handleReset = () => {
    if (window.confirm("Tum senkronize veriler sifirlansin mi?")) {
      localStorage.removeItem('insomni_synced_txs');
      setSyncedData([]);
      addLog("SISTEM: Tum veriler temizlendi.", "warning");
  ...
  const handleOcrScan = async () => {
    ...
    const newTx = { id: Date.now(), type: 'TRANSACTION', clean: "Apple Store Online", amount: "-45.990,00 TL", raw: "AAPL-9921 / TR", category: "LÜKS", source: "OCR ANALİZ", icon: 'Smartphone', color: "#6366f1", day: 15 };
  ...
  const processManual = async (text) => {
    ...
    const newTx = { id: Date.now(), type: 'TRANSACTION', clean: "Starbucks Coffee", amount: "-250,00 TL", raw: "VTM-3910 / ISTANBUL", category: "OTONOM", source: "OTONOM KÖPRÜ", icon: 'Coffee', color: "#00704A", day: 15 };
    setSyncedData(prev => [newTx, ...prev]);
    setIsSyncing(false);
  };
