...
export default function ScenarioSimulator({ currentIdleBalance = 0 }) {
  ...
  const scenarios = [
    { label: lang === 'tr' ? '10 GÃ¼nlÃ¼k VuruÅŸ' : '10-Day Impact', amount: 200000, days: 10 },
    { label: lang === 'tr' ? 'MaaÅŸ Tam Ã‡evrim' : 'Full Salary Cycle', amount: 100000, days: 30 },
    { label: lang === 'tr' ? 'Mevcut AtÄ±l Nakit' : 'Current Idle Cash', amount: currentIdleBalance, days: 30 },
  ];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        <div style={{ fontSize: 15, fontWeight: 700 }}>{lang === 'tr' ? 'BÃ¼yÃ¼k VuruÅŸ SenaryolarÄ±' : 'High-Impact Scenarios'}</div>
      </div>
          ...
          return (
            <motion.div key={i} whileHover={{ y: -4, background: 'var(--glass-bg-hover)' }}
              ...
              <div style={{ fontSize: 11, color: 'var(--text2)', marginBottom: 8, display: 'flex', justifyContent: 'space-between' }}>
                <span>{s.label}</span>
                <span style={{ fontWeight: 700, color: 'var(--accent)' }}>{s.days} {lang === 'tr' ? 'GÃœN' : 'DAYS'}</span>
              </div>
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 10, opacity: 0.6, marginBottom: 2 }}>{lang === 'tr' ? 'YatÄ±rÄ±m:' : 'Investment:'}</div>
                <div style={{ fontSize: 16, fontWeight: 800 }}>{t.currency}{s.amount.toLocaleString(lang === 'tr' ? 'tr-TR' : 'en-US')}</div>
              </div>
              <div>
                <div style={{ fontSize: 10, color: 'var(--green)', fontWeight: 700, marginBottom: 2 }}>{lang === 'tr' ? 'KAZANÃ‡' : 'POTENTIAL GAIN'}</div>
      ...
      <div style={{ fontSize: 11, color: 'var(--text2)', padding: '10px 14px', borderRadius: 10, background: 'var(--bg1)', border: '1px solid var(--glass-border)', display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        {lang === 'tr' 
          ? "Bu analizler gerÃ§ek Para PiyasasÄ± Fonu (PPF) ortalamalarÄ±yla hesaplanmÄ±ÅŸtÄ±r." 
