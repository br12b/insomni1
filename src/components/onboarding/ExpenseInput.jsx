...
export default function ExpenseInput({ onComplete }) {
  ...
  const INFO_CARDS = lang === 'tr' ? [
    {
      emoji: 'ÄŸÅ¸â€œâ€¦',
      title: 'Ãƒâ€“deme GÃƒÂ¼nÃƒÂ¼ Neden Ãƒâ€“nemli?',
      desc: 'MaaÃ…Å¸ gÃƒÂ¼nÃƒÂ¼n ile harcama gÃƒÂ¼nlerin arasÃ„Â±ndaki fark, nakdinin ne kadar sÃƒÂ¼re boÃ…Å¸ta beklediÃ„Å¸ini belirler. Bu sÃƒÂ¼reyi optimize ederek PPF getirisini artÃ„Â±rabilirsin.'
    },
    {
      emoji: 'ÄŸÅ¸â€â€',
      title: 'Abonelikler KÃƒÂ¼ÃƒÂ§ÃƒÂ¼k Ama Etkili',
      desc: 'AylÃ„Â±k 200-300 TL gibi gÃƒÂ¶rÃƒÂ¼nen abonelikler, yÃ„Â±llÃ„Â±k bazda 3.600 TL anlamÃ„Â±na gelir. Insomni bunlarÃ„Â± iÃ…Å¸aretleyerek sana tam gÃƒÂ¶rÃƒÂ¼nÃƒÂ¼rlÃƒÂ¼k saÃ„Å¸lar.'
    },
    {
      emoji: 'Ã¢Å¡Â¡',
      title: 'AtÃ„Â±l Nakit TuzaÃ„Å¸Ã„Â±',
      desc: "TÃƒÂ¼rkiye'de ortalama bir ÃƒÂ§alÃ„Â±Ã…Å¸an maaÃ…Å¸Ã„Â±nÃ„Â±n %40'Ã„Â±nÃ„Â± faizsiz hesapta tutuyor. Bu paranÃ„Â±n PPF'te deÃ„Å¸erlendirilmesi yÃ„Â±lda binlerce TL fark yaratÃ„Â±r."
    },
  ] : [
    {
      emoji: 'ÄŸÅ¸â€œâ€¦',
        ...
        <div>
          <div className="glass" style={{ padding: '40px 44px', marginBottom: 20 }}>
            ...
            <p style={{ color: 'var(--text2)', fontSize: 14, marginBottom: 28 }}>
              {lang === 'tr' ? 'Manuel girin ya da banka ekstrenizi yÃƒÂ¼kleyin.' : 'Enter manually or upload your bank statement.'}
            </p>
              ...
              {scanning ? (
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: 'var(--text2)' }}>
                    {lang === 'tr' ? 'PDF banka ekstreni bÃ„Â±rak veya tÃ„Â±kla' : 'Drop your bank statement PDF or click'}
                  </div>
            ...
            <div style={{ marginBottom: 20 }}>
              <div className="label" style={{ marginBottom: 10 }}>{lang === 'tr' ? 'HÃ„Â±zlÃ„Â± Ekle' : 'Quick Add'}</div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                <button onClick={() => addExpense()} className="chip"><Plus size={11} /> {lang === 'tr' ? 'BoÃ…Å¸ Ekle' : 'Empty'}</button>
              </div>
            </div>
                ...
                {expenses.map((exp) => (
                  <motion.div key={exp.id} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }}
                    ...
                    <div style={{ position: 'relative' }}>
                      ...
                      <input className="input" type="number" min="1" max="31" placeholder={lang === 'tr' ? 'GÃƒÂ¼n' : 'Day'} value={exp.date} onChange={e => update(exp.id, 'date', e.target.value)} style={{ fontSize: 13, padding: '10px 8px 10px 26px' }} />
                    </div>
